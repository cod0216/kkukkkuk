 ...
 * @notice 이 컨트랙트는 반려동물과 병원 간의 의료정보 공유를 위한 Address(탈중앙화 식별자를 대신함) 레지스트리입니다.
...
contract DIDRegistry {
    ...
    // 추가: 원본 의료기록 키 => 연결된 수정 기록 키 배열 (원본-수정 기록 매핑)
    mapping(string => string[]) private medicalRecordUpdates;
    
    // 추가: 반려동물 주소 => 원본 의료기록 키 배열 (반려동물별 원본 기록 목록)
    mapping(address => string[]) private petOriginalRecords;
    ...
    // 의료기록 추가 함수 - treatmentDate 파라미터 추가
    function addMedicalRecord(
        ...
        uint treatmentDate,          // 실제 진료 일자 (유닉스 타임스탬프, 0이면 block.timestamp 사용)
        string memory hospitalAccountId // 병원 계정 ID (빈 문자열 가능)
    ) public {
        // 접근 권한 확인 (소유자 또는 등록된 병원)
        bool userHasAccess = (msg.sender == owners[petAddress]);
        ...
        // 진료 일자 처리: treatmentDate가 0이면 현재 시간 사용
        uint actualTreatmentDate = treatmentDate > 0 ? treatmentDate : block.timestamp;
        ...
        // hospitalAccountId 추가를 위한 변수
        string memory hospitalAccountIdField = "";
        if (bytes(hospitalAccountId).length > 0) {
            hospitalAccountIdField = string(abi.encodePacked(',"hospitalAccountId":"', hospitalAccountId, '"'));
        }
        ...
        // 원본 의료기록 목록에 추가
        petOriginalRecords[petAddress].push(recordKey);
        
        // 속성으로 저장 (80년 유효)
        setAttribute(petAddress, recordKey, recordData, 80 * 365 days);
        
        // 기록 추가 이벤트 발생 (이전 기록 없음)
        emit MedicalRecordAdded(petAddress, recordKey, "", msg.sender, block.timestamp);
    }
    
    // 기존 addMedicalRecord 함수를 호환성을 위해 유지
    function addMedicalRecord(
        address petAddress,
        string memory diagnosis,
        string memory hospitalName,
        ...
        string memory medicationsJson,
        string memory vaccinationsJson,
        string memory picturesJson,
        string memory status,
        bool flagCertificated,
        uint treatmentDate
    ) public {
        // 새 함수를 호출하되 hospitalAccountId는 빈 문자열로 전달
        addMedicalRecord(
            petAddress,
            diagnosis,
            hospitalName,
    ...
    // 의료기록 추가(이전 기록 참조) 함수 - treatmentDate 파라미터 추가 및 Hub-Spoke 구조 변경
    function appendMedicalRecord(
        ...
        // 참조하는 기록이 원본 기록인지 확인 (새로운 Hub-Spoke 구조)
        require(isOriginalRecord(previousRecordKey), "Referenced record must be an original record");
        ...
        // 의료 기록 저장
        setAttribute(petAddress, recordKey, medicalData, 80 * 365 days);
        
        // 원본 기록과 수정 기록 매핑 업데이트
        medicalRecordUpdates[previousRecordKey].push(recordKey);
        
        // 기록 추가 이벤트 발생
        emit MedicalRecordAdded(petAddress, recordKey, previousRecordKey, msg.sender, block.timestamp);
    }
    
    // 기존 appendMedicalRecord 함수를 호환성을 위해 유지
    function appendMedicalRecord(
        address petAddress, 
        string memory previousRecordKey, 
        string memory diagnosis, 
    ...
    // 원본 의료기록인지 확인하는 함수
    function isOriginalRecord(string memory recordKey) public pure returns (bool) {
        // 원본 기록은 "medical_record_"로 시작
        bytes memory prefix = bytes("medical_record_");
        bytes memory keyBytes = bytes(recordKey);
        
        // 키가 prefix보다 짧으면 false
    ...
    // 원본 의료기록에 연결된 모든 수정 기록 키를 반환하는 함수
    function getMedicalRecordUpdates(string memory originalRecordKey) public view returns (string[] memory) {
        require(isOriginalRecord(originalRecordKey), "Not an original medical record");
        return medicalRecordUpdates[originalRecordKey];
    }
    
    // 반려동물의 모든 원본 의료기록 키를 반환하는 함수
    function getPetOriginalRecords(address petAddress) public view returns (string[] memory) {
        // 만료된 공유 계약을 처리하기 위해 hasAccess 대신 직접 확인
        bool userHasAccess = (msg.sender == owners[petAddress]);
        
        if (!userHasAccess) {
            // 공유 계약 확인
            SharingAgreement memory agreement = sharingAgreements[petAddress][msg.sender];
            if (agreement.exists) {
                ...
                // 만료된 계약은 접근 권한 없음
                // view 함수이므로 만료된 계약을 처리할 수는 없음, 단순히 접근만 거부
            } else {
                // 공유 계약이 없는 경우에만 병원 목록 확인
                address[] storage hospitalList = hospitals[petAddress];
                for (uint i = 0; i < hospitalList.length; i++) {
                    if (hospitalList[i] == msg.sender) {
                        userHasAccess = true;
        ...
        require(userHasAccess, "No permission to view records");
        return petOriginalRecords[petAddress];
    }
    ...
    function hasAccess(address petAddress, address user) public view returns (bool) {
        ...
        if (agreement.exists) {
            ...
            // 만료된 계약은 접근 권한 없음
            // 여기서 중요: 만료된 공유 계약이 있는 경우, 병원 목록에 있더라도 접근 권한 없음으로 처리
            return false;
        }
        
        // 공유 계약이 없는 경우에만 병원 목록 확인
        // 이미 만료된 공유 계약이 있는 경우 여기까지 오지 않음
        address[] storage hospitalList = hospitals[petAddress];
        for (uint i = 0; i < hospitalList.length; i++) {
    ...
    // 원본 의료기록과 모든 수정 기록을 함께 반환하는 함수
    function getMedicalRecordWithUpdates(address petAddress, string memory originalRecordKey) 
        public view returns (string memory originalRecord, string[] memory updateRecords) {
        ...
        // 원본 기록 조회
        (string memory record, ) = getAttribute(petAddress, originalRecordKey);
        
        // 수정 기록 키 배열 가져오기
        string[] memory updateKeys = medicalRecordUpdates[originalRecordKey];
        string[] memory updates = new string[](updateKeys.length);
        
        // 각 수정 기록 조회
        for (uint i = 0; i < updateKeys.length; i++) {
            (string memory updateRecord, ) = getAttribute(petAddress, updateKeys[i]);
            updates[i] = updateRecord;
        }
