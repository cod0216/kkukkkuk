...
import { faUserMd, faSpinner, faPlus, faTimes, faPencilAlt, faCheck, faTrashAlt, faExclamationTriangle } from '@fortawesome/free-solid-svg-icons';
import { fetchDoctors, Doctor as ApiDoctor, addDoctor, fetchDoctorDetail, updateDoctor, deleteDoctor } from '../../services/hospitalService';
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md mx-auto relative z-10">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-800 dark:text-gray-200">수의사 상세 정보</h2>
          <button 
            onClick={onClose}
        ...
        {isLoading ? (
          <div className="flex justify-center items-center py-8">
            ...
            <p className="text-gray-600 dark:text-gray-400">정보를 불러오는 중...</p>
          </div>
        ) : error ? (
          ...
          <div className="space-y-4">
            <div className="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg">
              ...
              <p className="text-lg font-medium text-gray-800 dark:text-gray-200">{doctor.id}</p>
            </div>
            <div className="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg">
              <p className="text-sm text-gray-500 dark:text-gray-400">이름</p>
              <p className="text-lg font-medium text-gray-800 dark:text-gray-200">{doctor.name}</p>
            </div>
            {/* 향후 추가될 필드를 위한 주석 처리 */}
            {/* 
            <div className="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg">
            ...
            */}
            <p className="text-xs text-gray-500 dark:text-gray-400 text-center mt-4">
              * 현재 서비스에서는 기본 정보만 제공됩니다. 추가 정보는 향후 업데이트될 예정입니다.
            </p>
          </div>
        ) : (
          <div className="text-center py-6">
...
const EditDoctorModal: React.FC<EditDoctorModalProps> = ({ isOpen, onClose, doctor, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md mx-auto relative z-10">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-800 dark:text-gray-200">수의사 정보 수정</h2>
          <button 
            onClick={onClose}
            className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">이름</label>
            <input
              type="text"
              value={name}
...
const DeleteDoctorModal: React.FC<DeleteDoctorModalProps> = ({ isOpen, onClose, doctor, onConfirm }) => {
  const [isProcessing, setIsProcessing] = useState(false);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md mx-auto relative z-10">
        ...
        <div className="mb-6">
          <p className="text-gray-700 dark:text-gray-300 mb-4">
            정말로 <span className="font-bold">{doctor.name}</span> 수의사를 삭제하시겠습니까?
          </p>
          <p className="text-gray-700 dark:text-gray-300 mb-4">
          ...
          <p className="text-sm text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-900/30 p-3 rounded-md">
            주의: 동물병원에는 최소 1명 이상의 수의사가 필요합니다. 현재 수의사가 1명뿐이라면 삭제할 수 없습니다.
          </p>
        </div>
    ...
    const loadDoctors = async () => {
      try {
        ...
        if (response.status === 'success' && response.data) {
          ...
          toast.error(response.message || '의사 목록을 가져오는데 실패했습니다.');
  ...
  const handleDeleteDoctor = async (id: string | number) => {
    try {
      ...
      if (response.status === 'success') {
        ...
        toast.success(response.message || '수의사가 성공적으로 삭제되었습니다.');
      } else {
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-6 dark:bg-gray-800">
      ...
      {isLoading ? (
        <div className="flex justify-center items-center py-8">
          <FontAwesomeIcon icon={faSpinner} spin className="mr-2 text-blue-500" />
          <p className="text-gray-600 dark:text-gray-400">의사 목록을 불러오는 중...</p>
        </div>
      ) : (
        <>
          <div className="mb-6">
            ...
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">* 필수 입력 항목</p>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300">이름</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300">전문 분야</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300">면허 번호</th>
                    <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300">관리</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                  {doctors.map((doctor: Doctor) => (
                    <tr key={doctor.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{doctor.specialization || "-"}</td>
                      <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{doctor.licenseNumber || "-"}</td>
