...
const IconMoreVertical =
  FiMoreVertical as unknown as React.ComponentType<IconProps>;
          ...
          <div className="flex items-center justify-between">
            ...
            <div
              onClick={(e) => {
                ...
                onUpdateTreatmentStatus(
                  pet.id,
                  getNextStatus(pet.treatmentStatus)
                );
              }}
              className={`px-2 py-1 rounded-full text-xs font-medium flex items-center ${getTreatmentStatusStyle(
                pet.treatmentStatus
              )}`}
            >
              {pet.treatmentStatus === "completed" && (
                <IconCheck size={12} className="mr-1" />
              )}
              {pet.treatmentStatus === "waiting" && (
                <IconClock size={12} className="mr-1" />
              )}
              {pet.treatmentStatus === "inProgress" && (
                <IconTarget size={12} className="mr-1" />
              )}
  ...
  return [...pets].sort((a, b) => {
    ...
    switch (sortField) {
      ...
      case "treatmentStatus":
        ...
        comparison =
          statusPriority[a.treatmentStatus] - statusPriority[b.treatmentStatus];
...
// 블록체인 상태 확인 함수
const getPetBlockchainStatus = (
  pet: Pet
): "synced" | "pending" | "error" | "none" => {
...
const PetList: React.FC = () => {
      ...
      if (response.status === "SUCCESS") {
        ...
        if (response.data.length > 0) {
          ...
          const petToSelect = previousSelectedId
            ? response.data.find((p: Pet) => p.id === previousSelectedId) ??
              response.data[0]
            : response.data[0];
        ...
        throw new Error(
          response.message || "반려동물 목록을 불러오는데 실패했습니다."
        );
      }
    } catch (err) {
      const errorMessage =
  ...
  let filteredPets = localPets.filter(
    (pet: Pet) =>
    ...
    <div className="h-full flex flex-col bg-white dark:bg-gray-800 shadow-sm">
      ...
      <div className="flex items-center text-xs px-4 py-1.5 bg-gray-50 dark:bg-gray-700 border-b dark:border-gray-600">
        ...
        >
          ...
          {sortField === "name" &&
            (sortOrder === "asc" ? (
              <IconArrowUp size={10} className="ml-1 inline" />
            ) : (
              <IconArrowDown size={10} className="ml-1 inline" />
            ))}
        </button>
        <button
