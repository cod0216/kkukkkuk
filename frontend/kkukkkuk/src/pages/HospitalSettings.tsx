...
import { selectLoggedInHospital, updateHospitalInfoAction, setHospitalInfo, logout as logoutAction } from '../redux/slices/authSlice';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faHospital, faIdCard, faPhone, faMapMarkerAlt, faEnvelope, faHome, faLock, faTrashAlt } from '@fortawesome/free-solid-svg-icons';
import { updateHospitalInfo, UpdateHospitalRequest, fetchHospitalMe, deleteHospitalAccount } from '../services/hospitalService';
    ...
    const fetchHospitalData = async () => {
      try {
        ...
        if (response.status === 'SUCCESS' && response.data) {
          ...
          toast.error(response.message || '병원 정보를 가져오는데 실패했습니다.');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === 'success' && result.data) {
        ...
        toast.success(result.message || '병원 정보가 성공적으로 업데이트되었습니다.');
      } else {
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-4xl">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">병원 설정</h1>
        <button
      ...
      {isFetching ? (
        <div className="flex justify-center items-center py-16">
          ...
          <p className="ml-3 text-lg text-gray-700 dark:text-gray-300">병원 정보를 불러오는 중...</p>
        </div>
      ) : (
        <>
          <div className="bg-white shadow-md rounded-lg p-6 mb-8 dark:bg-gray-800">
            ...
            <form onSubmit={handleSubmit}>
              <div className="grid md:grid-cols-2 gap-4 mb-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">병원명</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">DID 식별자</label>
                  <div className="relative">
                    <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-500 dark:text-gray-400">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">연락처</label>
                  <div className="relative">
                    <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-500 dark:text-gray-400">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">이메일</label>
                  <div className="relative">
                    <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-500 dark:text-gray-400">
              ...
              <div className="mb-6">
                <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">주소</label>
                <div className="relative">
                  <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-500 dark:text-gray-400">
                ...
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">주소는 변경할 수 없습니다.</p>
              </div>
              
              <div className="mb-6 border-t pt-6 border-gray-200 dark:border-gray-700">
                <h3 className="text-lg font-medium text-gray-700 mb-4 dark:text-gray-300">비밀번호 변경</h3>
                
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">새 비밀번호</label>
                    <div className="relative">
                      <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-500 dark:text-gray-400">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">비밀번호 확인</label>
                    <div className="relative">
                      <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-500 dark:text-gray-400">
                        <FontAwesomeIcon icon={faLock} />
                      </span>
                ...
                <div className="mt-2">
                  <label className="inline-flex items-center">
                    ...
                    />
                    <span className="ml-2 text-sm text-gray-600 dark:text-gray-400">비밀번호 보이기</span>
                  </label>
                </div>
              </div>
