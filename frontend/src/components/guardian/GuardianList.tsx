...
import { 
  ...
  TreatmentStatus,
  setCustomOrder
} from '../../redux/slices/guardianSlice';
import { fetchPetsByGuardian } from '../../redux/slices/petSlice';
import { Pet } from '../../redux/slices/petSlice';
import { 
  FiSearch, 
  FiUser, 
  FiPlus, 
...
import { DEV_USE_MOCK_BLOCKCHAIN } from '../../utils/blockchainUtils';
import { selectDevModeEnabled } from '../../redux/slices/devModeSlice';
...
// 정렬 가능한 보호자/반려동물 아이템 컴포넌트
interface SortablePetItemProps {
  guardian: Guardian;
  pet: Pet;
  isSelected: boolean;
  onSelect: (guardianId: string) => void;
  onUpdateTreatmentStatus: (guardianId: string, status: TreatmentStatus) => void;
...
const SortablePetItem: React.FC<SortablePetItemProps> = ({ 
  ...
  const getRemainingTime = () => {
    const now = new Date();
    const expiryDate = new Date(guardian.expiryDate);
  ...
  return (
    <li className={`${isSelected ? 'bg-blue-50 dark:bg-blue-900/20' : 'bg-white dark:bg-gray-800'} 
      ...
      ${guardian.treatmentStatus === 'inProgress' ? 'animate-blink' : ''}`}
      style={style}
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            ...
            <div 
              onClick={(e) => {
                e.stopPropagation();
                onUpdateTreatmentStatus(guardian.id, getNextStatus(guardian.treatmentStatus));
              }}
              className={`px-2 py-1 rounded-full text-xs font-medium flex items-center ${getTreatmentStatusStyle(guardian.treatmentStatus)}`}
            >
              {guardian.treatmentStatus === 'completed' && <IconCheck size={12} className="mr-1" />}
              {guardian.treatmentStatus === 'waiting' && <IconClock size={12} className="mr-1" />}
              {guardian.treatmentStatus === 'inProgress' && <IconTarget size={12} className="mr-1" />}
              {getTreatmentStatusText(guardian.treatmentStatus)}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between text-xs">
            <div className="flex items-center text-gray-500 dark:text-gray-400">
              ...
              <span>{formatDate(guardian.registeredDate)}</span>
            </div>
...
// 보호자 정렬 함수 (진료 상태 포함)
const sortGuardians = (guardians: Guardian[], sortField: SortField, sortOrder: SortOrder, customOrder: string[] = []) => {
  // 커스텀 정렬인 경우
...
// 보호자별 반려동물 검색 및 블록체인 상태 확인 함수
const getGuardianPetInfo = (guardianId: string, petList: Pet[], devModeEnabled: boolean) => {
  const guardianPets = petList.filter(pet => pet.guardianId === guardianId);
  const petCount = guardianPets.length;
  const firstPet = guardianPets[0] || null;
  
  // 개발 환경에서는 항상 'synced' 상태로 처리
  if (devModeEnabled) {
    return { firstPet, petCount, blockchainStatus: 'synced' as const };
  }
  
  // 블록체인 상태 확인
  let blockchainStatus: 'synced' | 'pending' | 'error' | 'none' = 'none';
  
  if (petCount > 0) {
    // 모든 반려동물의 블록체인 데이터 확인
    const hasBlockchainData = guardianPets.some(pet => pet.blockchainData);
    const hasError = guardianPets.some(pet => pet.blockchainData && new Date(pet.blockchainData.timestamp).getTime() < Date.now() - 86400000 * 7); // 7일 이상 된 데이터
  ...
  return { firstPet, petCount, blockchainStatus };
};

const GuardianList: React.FC = () => {
  const [searchTerm, setSearchTerm] = useState('');
  const [localGuardians, setLocalGuardians] = useState<Guardian[]>([]);
  const dispatch = useAppDispatch();
  const { 
    guardians: reduxGuardians, 
    selectedGuardian: reduxSelectedGuardian, 
    loading, 
    error, 
    sortField, 
    sortOrder,
    hideCompleted,
    customOrder
  } = useAppSelector((state: RootState) => state.guardian);
  ...
  const loadGuardians = useCallback(() => {
    ...
    try {
      ...
      // 개발 모드에서는 현재 상태를 그대로 사용하여 불필요한 초기화를 방지
      if (devModeEnabled && reduxGuardians.length > 0) {
        dispatch(fetchGuardiansSuccess(reduxGuardians));
        return;
      }
      ...
      setTimeout(() => {
        const stateGuardians = store.getState().guardian.guardians;
        dispatch(fetchGuardiansSuccess(stateGuardians));
        
        // 가디언 로딩 시 초기 선택 가디언 설정
        if (stateGuardians.length > 0) {
          // 이전에 선택된 가디언이 있으면 해당 가디언 유지, 아니면 첫 번째 가디언 선택
          const previousSelectedId = localStorage.getItem('selectedGuardianId');
          const guardianToSelect = previousSelectedId 
            ? stateGuardians.find(g => g.id === previousSelectedId) ?? stateGuardians[0]
            : stateGuardians[0];
            
          dispatch(selectGuardian(guardianToSelect.id));
          
          // 선택된 가디언 ID를 로컬 스토리지에 저장
          localStorage.setItem('selectedGuardianId', guardianToSelect.id);
        }
      }, 500);
    } catch (err) {
      dispatch(fetchGuardiansFailure('보호자 정보를 불러오는데 실패했습니다.'));
      toast.error('보호자 정보를 불러오는데 실패했습니다.');
    }
  }, [dispatch, devModeEnabled, reduxGuardians, loading]);

  useEffect(() => {
    // 개발 모드인 경우 초기 로드 이후에는 loadGuardians를 호출하지 않음
    if (!devModeEnabled || reduxGuardians.length === 0) {
      loadGuardians();
    }
  }, []);
  ...
  useEffect(() => {
    const sortedGuardians = sortGuardians(reduxGuardians, sortField, sortOrder, customOrder);
    setLocalGuardians(sortedGuardians);
  }, [reduxGuardians, sortField, sortOrder, customOrder]);
  ...
  const handleGuardianSelect = (guardianId: string) => {
    ...
    // 선택한 보호자를 찾음
    const guardian = reduxGuardians.find(g => g.id === guardianId);
    
    if (guardian) {
      // 선택한 보호자 ID를 상태에 저장
      dispatch(selectGuardian(guardianId));
      
      // 로컬 스토리지에 선택한 보호자 ID 저장
      localStorage.setItem('selectedGuardianId', guardianId);
      
      // 선택한 보호자의 반려동물 목록 조회
      dispatch(fetchPetsByGuardian(guardianId));
    }
  };

  const handleUpdateTreatmentStatus = (guardianId: string, status: TreatmentStatus) => {
    dispatch(updateTreatmentStatus({ guardianId, status }));
  ...
  // 필터링된 보호자 목록 (완료 상태인 경우에만 숨김)
  let filteredGuardians = localGuardians.filter((guardian: Guardian) => 
    (guardian.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    guardian.email.toLowerCase().includes(searchTerm.toLowerCase())) &&
    (!hideCompleted || guardian.treatmentStatus !== 'completed')
  );
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2">
        ...
        {!loading && !error && (
          <>
            ...
            >
              <SortableContext
                items={filteredGuardians.map(guardian => guardian.id)}
                strategy={verticalListSortingStrategy}
              >
                <ul>
                  {filteredGuardians.map(guardian => {
                    const { firstPet, blockchainStatus } = getGuardianPetInfo(guardian.id, pets, devModeEnabled);
                    // 반려동물이 있는 경우에만 표시
                    ...
                    return (
                      <SortablePetItem
                        ...
                        guardian={guardian}
                        pet={firstPet}
                        isSelected={reduxSelectedGuardian?.id === guardian.id}
            ...
            {/* 결과 없음 표시 */}
            {filteredGuardians.filter(g => pets.some(p => p.guardianId === g.id)).length === 0 && (
