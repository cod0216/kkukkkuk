...
import { BlockChainRecord } from "@/interfaces/index";
  ...
  return (
    ...
    >
        ...
        <div className="text-xs text-gray-500 flex gap-2 items-center">
          {examinations > 0 && (
            <span className="flex items-center gap-1">
              {" "}
              <span className="rounded-max w-1.5 h-1.5 bg-red-200"></span>{" "}
              {examinations}
            </span>
          )}
          {medications > 0 && (
            <span className="flex items-center gap-1">
              {" "}
              <span className="rounded-max w-1.5 h-1.5 bg-secondary-200"></span>
              {medications}
            </span>
          )}
          {vaccinations > 0 && (
            <span className="flex items-center gap-1">
              {" "}
              <span className="rounded-max w-1.5 h-1.5 bg-primary-300"></span>
              {vaccinations}
            </span>
          )}
        </div>
      ...
      <div className="flex flex-col items-end text-right">
        <div
          className={`${
            isMyRecord ? `bg-primary-200` : `bg-secondary-100`
          } text-[8pt] text-gray-600 font-medium rounded-xl  px-2`}
        >
          {record.hospitalName}
        </div>
        <div className="font-md text-[8pt] text-gray-500 px-1">
          {record.createdAt}
        </div>
      </div>
    </div>
