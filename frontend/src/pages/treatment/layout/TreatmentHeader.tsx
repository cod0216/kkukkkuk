...
const TreatmentHeader: React.FC<TreatmentHeaderProps> = ({ 
  treatment, getStateBadgeColor, isFormVisible, onSelected 
}) => {
  return (
    <div className="bg-white p-4 rounded-lg border mb-3 flex items-center">
      ...
      <div className="flex-1 px-4">
        <div className="flex items-center gap-3">
          <div className="text-md font-bold text-gray-800">{treatment?.name}</div>
          {treatment && (
            <div className={`text-[10px] px-2 py-0.5 rounded-full ${getStateBadgeColor(treatment.state)}`}>
              {treatment?.state}
            </div>
          )}
        </div>
        <div className="text-gray-600 text-sm font-medium mt-0.5">{treatment?.breedName}</div>
        <div className="text-gray-500 text-xs flex gap-1 mt-1">
          <span>{treatment?.gender === Gender.MALE ? '수컷' : '암컷'}</span>
          <span>·</span>
          <span>{treatment?.flagNeutering ? '중성화 완료' : '중성화 안함'}</span>
          <span>·</span>
